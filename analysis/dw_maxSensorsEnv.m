...
modelPath = fullfile(pathstem,'TRF','env_encoding');
modelFiles = dir(fullfile(modelPath,'modelAccuracy_subj*.mat'));
...
for s=1:numel(modelFiles) % loop through model data files
    ...
    % save
    save(fullfile(modelPath,['Env_maxSensors_subj' num2str(s) '.mat']),'maxSensors');
