...
analysis_folder = fullfile(pathstem,'TRF','ph_encoding');
maxSensorsFiles = dir(fullfile(analysis_folder,'PhonemeSurprisal_maxSensors_subj*.mat'));
fs_new = 64;
...
%         indChannels = D.selectchannels('EEG');
        load(fullfile(analysis_folder,maxSensorsFiles(s).name));
        indChannels = maxSensors; % use best 20 EEG channels instead
        ...
        current_stim = stim{1,(chapter)}; % load predictability values for current chapter
        if triggerVals(2) == 95
            condition = 'Audio';
        else
            condition = 'Visual';
...
%         current_stim = [zeros(speechOnset-1,2); current_stim];
        current_stim = [zeros(speechOnset-1,1); current_stim];
...
%     saveas(100,fullfile(analysis_folder,['regressors_' subject2process '_b' blocks2process{b} '_chapter' num2str(chapter) '_' condition '_' MOT_load '.png']));
    ...
    save(fullfile(analysis_folder,['maxSurprisal_modelAccuracy_' subject2process '.mat']),'rmax_audio','rmax_visualLow','rmax_visualHigh','lambda','cv_audio','cv_visualLow','cv_visualHigh','lambdas','tmin','tmax','average_audio_model','average_visualLow_model','average_visualHigh_model');
    saveas(200,fullfile(analysis_folder,['maxSurprisal_modelAccuracy_' subject2process '_Audio' '.png']));
    saveas(300,fullfile(analysis_folder,['maxSurprisal_modelAccuracy_' subject2process '_VisualLow' '.png']));
    saveas(400,fullfile(analysis_folder,['maxSurprisal_modelAccuracy_' subject2process '_VisualHigh' '.png']));
    saveas(500,fullfile(analysis_folder,['maxSurprisal_modelWeights_' subject2process '_Audio' '.png']));
    saveas(600,fullfile(analysis_folder,['maxSurprisal_modelWeights_' subject2process '_VisualLow' '.png']));
    saveas(700,fullfile(analysis_folder,['maxSurprisal_modelWeights_' subject2process '_VisualHigh' '.png']));
